...
class handDetector():
    ...
    def findHands(self,img, draw=True, needConversionToRGB=True):
        if needConversionToRGB:
    ...
    def drawHandsAndGetVolume(self, img, draw=True, needConversionToRGB=True):
        ...
        if len(lmList) != 0:
            ...
            # Volume Range -65 - 0
            handRange = (16,270)
            vol = np.interp(length, [handRange[0], handRange[1]], [0, 1.0])
            volBar = np.interp(length, [handRange[0], handRange[1]], [400, 150])
            volPer = np.interp(length, [handRange[0], handRange[1]], [0, 100])
